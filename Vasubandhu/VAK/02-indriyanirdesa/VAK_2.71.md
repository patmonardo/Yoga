# VAK_2.71 — Twelve Cittas and the Wholesome Split

## 1. Sanskrit — Devanāgarī

```text
तस्माच्चत्वारि चित्तानि द्वादशैतानि विंशतिः ।
प्रायोगिकोपपत्त्याप्तं शुभं भित्त्वा त्रिषु द्विधा ॥ VAkK_2.71 ॥
```

## 2. Sanskrit — IAST

```text
tasmāc catvāri cittāni dvādaśaitāni viṃśatiḥ /
prāyogikopapattyāptaṃ śubhaṃ bhittvā triṣu dvidhā // VAkK_2.71 //
```

## 3. Literal Translation

```text
From that, four cittas. These are twelve; twenty when the wholesome is divided in the three realms into practice-acquired and rebirth-acquired.
```

## 4. Philosophical Translation

```text
The citta matrix is summarized and expanded.
The basic twelvefold citta classification becomes twentyfold when wholesome citta in the three realms is divided into practice-acquired and rebirth-acquired types.
```

## 5. Technical Vocabulary

| Sanskrit | Working Translation | Note |
|---|---|---|
| dvādaśa | twelve | Basic citta classification count |
| viṃśati | twenty | Expanded citta classification count |
| prāyogika | practice-acquired / application-produced | Cultivation-produced status |
| upapattyāpta | rebirth-acquired | Obtained by birth in a realm |
| śubha | wholesome | Wholesome citta |
| bhittvā | having divided | Classification split |
| triṣu | in the three | Three realms |
| dvidhā | twofold | Twofold split |

## 6. Logical Determination

```text
BasicCittaMatrix:
    total = 12

ExpandedCittaMatrix:
    wholesome in three realms split into:
        practice-acquired
        rebirth-acquired
    total = 20
```

This is classification expansion by acquisition-mode.

## 7. Organon Interpretation

```text
CittaClassExpansion:
    baseClasses = 12
    expandedClasses = 20
    splitCriterion = acquisitionMode
```

The Dataset must permit feature-class refinement without losing the base matrix.

## 8. Commit History

```text
VAK_2.71 opened.
Method: kārikā-first speculative seed.
Theme: twelvefold citta matrix expanded to twenty through wholesome acquisition-mode split.
```
