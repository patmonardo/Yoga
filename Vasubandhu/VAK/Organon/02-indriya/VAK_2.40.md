...
iṣyate              → iṣyate
pṛthagjanatvam      → pṛthagjanatvam
tatprāptibhūsaṃcārāt
                    → tat-prāpti-bhū-saṃcārāt
vihīyate            → vihīyate
```
...
| iṣyate | present passive third-person singular | is held / accepted |
| pṛthagjanatvam | nominative neuter singular | ordinary-person status |
| tat-prāpti | compound member | acquisition of that [dharma or Path] |
| bhū-saṃcāra | compound member | transition to another ground |
| -āt | ablative singular ending | because of / through |
...
The opening genitive phrase presupposes the predicate `aprāptiḥ tridhā`
from the preceding doctrinal unit:
...
```text
mārgasya aprāptiḥ pṛthagjanatvam iṣyate
    → non-acquisition of the Path is held
      to be ordinary-person status
```

The final compound compresses two instrumental causes of abandonment. The
Bhāṣya unfolds them disjunctively:

```text
tat-prāpti
    acquisition of that dharma, especially the noble Path

bhū-saṃcāra
    transition to another ground
```

`Vihīyate` expresses a formal cessation of status, not a merely psychological
act of renunciation.
...
> Ordinary-person status is a developmental determination: the noble Path is not yet operative in this continuum. The Vaibhāṣika system represents that fact through a real `aprāpti`; the Sautrāntika account represents it genetically as the non-arising of noble dharmas. In either model, the status is not an essence of the person. It is left behind when the continuum acquires the Path or undergoes a ground-transition.

The Bhāṣya then tests the cost of treating acquisition and non-acquisition
as real entities. If a dharma's possession requires a `prāpti`, possession of
that `prāpti` appears to require a `prāpti-prāpti`. The proposed solution of
...
| parasparasamanvāgama | mutual possession | Vaibhāṣika proposal for stopping the explicit regress |

`Pṛthagjana` is rendered “ordinary person” rather than “worldling” so that
the technical path-status is not confused with a moral insult or permanent
personal nature.
...
```text
Pṛthagjanatva(S)
    := RealAprapti(S, NoblePath)
```

The Sautrāntika/Vasubandhu alternative is genetic:
...
```text
RealPrapti(P, S, D)
    → requires possession of P
    → RealPrapti(P2, S, P)
```

The Vaibhāṣika mutual-possession reply adds:

```text
P possesses D and P2
P2 possesses P
```

It avoids a simple linear regress but generates temporally proliferating
acquisition-substances. Vasubandhu's criticism is therefore ontological
inflation, not merely formal non-termination.
...
vak:VAK_2_40
    a vak:Karika ;
    rdfs:label "VAK 2.40" ;
    vak:hasTopic vak:PathAprapti,
        vak:Pṛthagjanatva,
        vak:ApraptiAbandonment ;
    vak:belongsTo vak:Indriyanirdesa .

vak:VaibhasikaOrdinaryStatus
    a vak:AttributedDoctrinalPosition ;
    vak:defines vak:Pṛthagjanatva ;
    vak:through vak:RealApraptiOfNoblePath .

vak:SautrantikaOrdinaryStatus
    a vak:AttributedDoctrinalPosition ;
    vak:defines vak:Pṛthagjanatva ;
    vak:through vak:NonArisingOfNobleDharmasInContinuum .

vak:ApraptiAbandonment
    vak:occursThrough vak:PraptiOfCorrespondingDharma,
        vak:GroundTransition .

vak:VaibhasikaSecondOrderPrapti
    a vak:AttributedDoctrinalPosition ;
    vak:asserts vak:PraptiPrapti,
        vak:MutualPossession .

vak:VasubandhuProliferationCritique
    a vak:AttributedDoctrinalPosition ;
    vak:targets vak:ReifiedPraptiOntology ;
    vak:identifies vak:IndefiniteEntityProliferation .

organon:DerivedPathStatus
    a organon:InterpretiveReconstruction ;
    organon:isDerivedFrom vak:NobleDharmaArising,
        vak:GroundTransition ;
