...
output_file="$edition_dir/Organon-Kosa-Karma-Bhasya-Nirdesa-Part-Two.pdf"

sources=()
for index in $(seq 13 66); do
...
pandoc "${sources[@]}" \
  ...
  --metadata title="The Organon Kośa: Karmanirdeśa Bhāṣya, Part Two" \
  --metadata subtitle="A Continuous Translation and Focused Study" \
  --metadata author="The Organon Project" \
  --metadata date="Provisional Work in Progress · VAK 4.13–4.66" \
